 ...
 * Constants
 */
static rocksdb::WriteOptions asyncWriteOptions;
static rocksdb::WriteOptions syncWriteOptions;
static rocksdb::ReadOptions defaultReadOptions;
static rocksdb::Options databaseOptions;

/**
 * Initializes the object store.
 */
RocksDbObjectStore::RocksDbObjectStore() {
...
void RocksDbObjectStore::flush() {
    ...
    rocksdb::Slice flushDataKey(globalConfig.flushDataKey());
    rocksdb::Status status = m_database->Get(defaultReadOptions, flushDataKey, &serializedEntryData);
     ...
     */
        ...
        rocksdb::Slice slicedData(serializedEntryData);
        batch.Delete(globalConfig.flushDataKey());
...
void RocksDbObjectStore::optimizeMedia() {
    ...
    m_database->CompactRange(compactRangeOptions, nullptr, nullptr);
}
...
void RocksDbObjectStore::putEntry(const Command_KeyValue& params, const string& value) {
    ...
    if (!params.force()) {
        string serializedEntryData;
        rocksdb::Status status = m_database->Get(defaultReadOptions, params.key(), &serializedEntryData);
    ...
    rocksdb::Status status = m_database->Put(getWriteOptions(params.synchronization()), params.key(), serializedData);
...
void RocksDbObjectStore::deleteEntry(const Command_KeyValue& params) {
    ...
    if (!params.force()) {
        string serializedEntryData;
        rocksdb::Status status = m_database->Get(defaultReadOptions, key, &serializedEntryData);
     ...
     * specified.
     */
    rocksdb::Status status = m_database->Delete(getWriteOptions(params.synchronization()), key);
...
void RocksDbObjectStore::getEntry(const string& key, string& returnValue, Command_KeyValue* returnMetadata) {
     ...
     */
    string serializedEntryData;
    rocksdb::Status status = m_database->Get(defaultReadOptions, slice, &serializedEntryData);
...
void RocksDbObjectStore::getNextEntry(const string& key, string& returnValue, Command_KeyValue* returnMetadata) {
     ...
     */
    rocksdb::Iterator* iterator = m_database->NewIterator(defaultReadOptions);
    iterator->Seek(key);
 ...
 */
inline rocksdb::WriteOptions& RocksDbObjectStore::getWriteOptions(Command_Synchronization option) {
 ...
 */
BatchInterface* RocksDbObjectStore::createBatch() {
    return new RocksDbBatch(this);
}
